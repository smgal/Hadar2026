...
namespace Yunjr
{
	public class YunjrMap_T1_Origin : YunjrMap
	{
		...
		public override void OnLoad(string prev_map, int from_x, int from_y)
		{
			...
			else if (prev_map == "Map002")
			{
				GameRes.party.Warp(50, 26);
				GameRes.party.SetDirection(0, 1);
			}
			else
			{
				...
				// 임시
				// 감옥
				// GameRes.party.Warp(50, 10);
				// 중심
				// GameRes.party.Warp(58, 78);
				// Flag_Set(31);
			}

			// 이벤트 -> 생성 { 동료 추가 푯말 + 로드안 뒤의 테스트용 던젼 입구 }
			GameRes.map_data.data[47, 26].act_type = ACT_TYPE.EVENT;
			GameRes.map_data.data[50, 29].act_type = ACT_TYPE.EVENT;

			// 로드안 바로 앞에서 헤더 메시지가 나오는 이벤트
			GameRes.map_data.data[49, 29].act_type = ACT_TYPE.EVENT;
			GameRes.map_data.data[50, 29].act_type = ACT_TYPE.EVENT;
			GameRes.map_data.data[51, 29].act_type = ACT_TYPE.EVENT;
			...
			// 임시로 만든 로어성 입구의 푯말
			/*
			GameRes.map_data.data[48, 58].ix_obj1 = 112;
			GameRes.map_data.data[48, 58].act_type = ACT_TYPE.SIGN;
			GameRes.map_data.data[52, 58].ix_obj1 = 112;
			GameRes.map_data.data[52, 58].act_type = ACT_TYPE.SIGN;
			*/
			
			// 감옥 앞의 제작자
			{
				GameRes.map_data.data[45, 8].ix_obj1 = 136;
				GameRes.map_data.data[45, 8].act_type = ACT_TYPE.TALK;
				/*
				Tile_CopyToDefaultTile(44);
				Tile_CopyToDefaultSprite(24);
				Map_ChangeTile(45, 8, CONFIG.TILE_BG_SPECIAL);*/
			}
		...
		public override void OnTalk(int event_id)
		{
			if (On(47, 30))
			{
				new Npc.ItemShop(this, Npc.ItemShop.ITEM.MEDICINE);
				return;
			}
