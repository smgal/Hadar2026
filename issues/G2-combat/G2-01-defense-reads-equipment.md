...
- [ ] `battle.dart:514` 의 `t.ac` 가 **장비 합산 `ac`** 를 읽는다 (= [G1-05](../G1-items/G1-05-equipment-effect.md) 의 항등식이 전투 진입 시점에 성립)
- [ ] **`battle.dart` 가 한 줄도 바뀌지 않았다** (`git diff --stat lib/application/battle.dart` = 변경 0)
- [ ] `battle.dart:441` 이 적 방어이며 장비와 무관함이 **주석으로 명시**되었다 (초판 오독의 재발 방지)
- [ ] 방패·갑옷 `ac` 가 원작 대역(방패 0~5 / 갑옷 0~10)이고, 초기 장비 합산이 **파티 소양 3~5 와 같은 자리**임을 계산표로 확인했다
- [ ] `ac ∈ {3,5,7,10,15,20,25}` 의 "피해 발생 턴 비율" 표를 **부록 H-2 형식으로** 이슈에 기록했다
- [ ] `powOfShield`/`powOfArmor` 가 방어에 무관함이 코드 주석에 남아 있다 ([P0-19](../P0-foundation/P0-19-dead-equipment-fields.md) 와 중복되지 않게, 이 이슈는 **확인만**)
- [ ] `books.json` 이 수정되지 않았다
- [ ] 테스트 추가: `hadar2026_app/test/application/defense_scale_test.dart` —
