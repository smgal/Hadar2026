...
## G1 — 아이템·장비 이식 (착수 가능 · 자동화와 무관하게 필요)
...
| ID | 제목 | 상태 | 규모 | 선행 |
|---|---|---|---|---|
| [G1-01](G1-items/G1-01-item-model-port.md) | `Item`/`ItemSub`/`ITEM_TYPE` 을 `domain/item/` 으로 이식 | TODO | M | 없음 |
| [G1-02](G1-items/G1-02-item-data.md) | 아이템 실데이터 확보 (`books.json` 은 무기5·방어구3 샘플뿐) | TODO | M | G1-01 |
| [G1-03](G1-items/G1-03-party-inventory.md) | 파티 소지품 — `HDParty` 에 아이템 목록 | TODO | M | G1-01 |
| [G1-04](G1-items/G1-04-equipment-slots.md) | 장비 슬롯 재편 — 정수 3칸 → 부위별(`ARMOR`/`HEAD`/`LEG`/`ORNAMENT` 포함) | TODO | M | G1-01 |
| [G1-05](G1-items/G1-05-equipment-effect.md) | 장비 효과 배선 — `powOfWeapon ← atta_pow` · `ac` 합산 | TODO | M | G1-04 |
| [G1-06](G1-items/G1-06-item-names.md) | `"무기1"` → 실제 이름 (노출 4곳) | TODO | S | G1-02 |
| [G1-07](G1-items/G1-07-inventory-ui.md) | 소지품·장비 화면 (`GameEventEquipment.cs` 참조) | TODO | L | G1-03·G1-04 |
| [G1-08](G1-items/G1-08-cm2-item-commands.md) | cm2 커맨드 `Item::Give/Take/Has` | TODO | S | G1-03 |
| [G1-09](G1-items/G1-09-item-save.md) | 세이브에 소지품·장비 포함 | TODO | M | G1-03·G1-04 |
| [P0-19](P0-foundation/P0-19-dead-equipment-fields.md) | `powOfShield`/`powOfArmor` 죽은 필드 정리 | TODO | S | G1-05 |

## G2 — 전투 정합 (G1 일부와 병행 가능)

| ID | 제목 | 상태 | 규모 | 선행 |
|---|---|---|---|---|
| [G2-01](G2-combat/G2-01-defense-reads-equipment.md) | 방어 계산이 장비에서 온 `ac` 를 읽게 한다 | TODO | M | G1-05 |
| [G2-02](G2-combat/G2-02-unregistered-cm2-symbols.md) | 미등록 cm2 심볼 6종 9곳 정리 (`Party::CheckIf` 등) | TODO | M | 없음 |
| [P0-12](P0-foundation/P0-12-battle-result-inverted.md) | 전투 결과 코드가 cm2 상수와 반대 | TODO | S | 없음 |
| [P0-13](P0-foundation/P0-13-battle-result-defaults-win.md) | `Battle::Result()` 가 전투 없이 승리 반환 | TODO | S | P0-12 |
| [P0-15](P0-foundation/P0-15-enemy-id-zero.md) | 적 id 0 영구 소환 불가 — 유효 범위 1~74 | TODO | S | 없음 |

## S1 — 샘플 퀘스트 (G1 완료 후 · 코드 변경 0)

| ID | 제목 | 상태 | 규모 | 선행 |
|---|---|---|---|---|
| [S1-01](S1-sample-quest/S1-01-quest-design.md) | 샘플 퀘스트를 설계한다 (원작 톤·3단계·플래그 배분) | BLOCKED | S | G1 |
...
| ID | 제목 | S1 중 물릴 위험 | 근거 |
|---|---|---|---|
| [P0-03](P0-foundation/P0-03-cm2-load-state-leak.md) | cm2 로드 실패가 엔진 상태를 누수시킨다 | **높음** — 오타로 cm2 로드가 실패하면 이전 스크립트가 남아 원인 추적 불가 | 부록 A-2 |
| [P0-14](P0-foundation/P0-14-silent-out-of-range.md) | 범위 밖 인자가 조용히 무시된다 | **높음** — 플래그 인덱스 오타가 침묵 | 부록 F-1 |
