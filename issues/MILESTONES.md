...
## 8. W 트랙 — RPG 핵심 재작성 (2026-09-08 신설, [10차 판정](DECISION-LOG.md))

### 8.1 왜 새로 쓰는가

BP-44~47 을 쓰면서 **지금 모양에 안 들어가는 것**이 넷 나왔다:

| 어긋남 | 어디 |
|---|---|
| 장비 부위가 단수 함수라 쌍수·횃불이 들어갈 자리가 없다 | 부록 Z-5 |
| 파티 버프가 저장값인데 계산값이어야 한다 | 부록 Z-4 |
| 인물이 문자열 63가지를 분기해 `hands`·`shape`·`classMask` 를 붙일 데가 없다 | `player.dart` 735줄 |
| 타일 열거가 지형과 스크립트 모드를 겸직한다 | `tile_properties.dart` |

그리고 **다시 쓸 부분에 테스트가 없다** — 마법 시전 0건 · 타일 이벤트 0건 · 메뉴 흐름 1건.
부록 Z 의 세 건(절벽이 평지 · 늪이 안 아픔 · 마법 35·36 이 아무것도 안 함)이 그 결과다.
지킬 동작이 없으니 회귀 위험도 없다.

### 8.2 산출물 — 독립 실행되는 디렉토리 셋

```
packages/hd_world       모델. Flutter · dart:io · 이 레포의 다른 패키지 전부 금지
packages/hd_world_text  한국어 이름표
hd_world_lab            OpenAPI 서버 + 마우스 화면. 규칙 없음
```

`hd_battle` · `hd_battle_text` · `hd_battle_console` 과 같은 배치이고, 같은 이유다.

### 8.3 무엇이 이 트랙에 들어 있지 않은가

**지도 · 대화 · 스크립트 · 세계 진행.** 전투는 입력·출력이 명확했지만 RPG 는 자연
경계가 없어서 "전체 재작성" 으로 번지기 쉽다. 그래서 패키지를 **빼는 것으로 정의**했다 —
Flutter 없음 · 렌더링 없음 · cm2 없음 · 에셋 없음. `presentation/` 3,298줄은 손대지 않는다.

### 8.4 B7·B8·B9 를 대신한다

BP-44 §8 · BP-45 §7 · BP-47 §9 가 제안한 세 트랙이 전부 같은 파일 여섯 개를 만진다.
세 번 고치는 대신 한 번 다시 쓴다. **적 명세(BP-44 §1~§6)만 별개**다 — 그쪽은
`hd_battle` 의 표이고 아군 모델과 무관하다.

### 8.5 이 구간이 찾아낸 결함 셋

다시 쓰는 동안 **한 번도 발동한 적 없는 결함 셋**이 드러났다. 이것이 재작성의
부수 소득이다 — 옛 코드를 고치는 것만으로는 나오지 않았을 것들이다.

| | 무엇 | 왜 안 보였나 |
|---|---|---|
| [Z-7](../blueprint/_meta/GROUND_TRUTH.md) | 출하 스크립트가 원작의 **17직업 번호**를 쓰는데 앱은 안 썼다 — cm2 가 직업을 정하면 「알 수 없음」이 나왔다 | 직업을 읽는 곳이 표시뿐이었다 |
| Z-8 | **레벨이 오르면 최대 체력이 줄었다** — 슴갈 150 → 34 | `checkLevelUp` 호출처가 죽은 코드뿐이라 발동한 적이 없다 |
| Z-9 | 초능력 41·42·44 가 전투 메뉴에 있고 **아무 일도 하지 않았다** — B5 불변식(헛턴 금지) 위반 | 마법 33~40 은 같은 이유로 빠졌는데 셋만 남았다 |

Z-9 는 **B3-04 의 답도 준다** — `worldEffects` 를 채우는 경로가 전투에 하나도 없으므로
해석할 요청이 애초에 없다.

### 8.6 진행 (2026-09-09 — **완료**)

| | 무엇 | 상태 |
|---|---|---|
| W1-01 | 패키지 셋 · 독립성 게이트 · CI | **DONE** |
| W1-02 | 부위 여덟 · 아이템 31종 · 자격 검사 | **DONE** |
| W1-03 | 파생값 무저장 | **DONE** |
| W1-04 | 직업 17 이식 · 상시 지시 7 유도 | **DONE** |
| W1-05 | OpenAPI 표면 · 마우스 화면 | **DONE** |
| W1-06 | 전투 다리 (**규격 v4**) | **DONE** |
| W1-07 | 세이브 포맷 | **DONE** |
| W1-08 | cm2 어댑터 | **DONE** |
| W1-09 | 앱 교체 | **DONE** |

**앱이 새 모델 위에서 돈다.** 함께 닫힌 것: **B4-02**(전투 화면을 게임 안으로 —
실험실과 같은 위젯) · **B4-03**(옛 전투 809줄 제거) · **B3-04**(해석할 요청이 없다).

시험 **917개** — 모델 112 · 이름표 4 · 옛 어휘 23 · 다리 18 · 전투 392 ·
전투 이름표 18 · cm2 42 · 실험실 14 · 콘솔 114 · 앱 180.
자세한 것은 [W1 _README](W1-world-core/_README.md).
