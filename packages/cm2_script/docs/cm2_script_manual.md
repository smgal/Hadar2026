...
- `JoinString(s1, s2, ...)`: 인자들을 순서대로 이어 붙인 하나의 문자열을 반환합니다. (내장 함수)

### 4.6 Context (컨텍스트)

엔진은 **이름 붙은 context**를 여러 개 가질 수 있고, 그중 하나가 **현재 context**입니다. 현재 context 안에서만 키–값을 넣고 꺼냅니다.

- **설정/전환**
  - `Context::SetCurrent(name)`: 이름이 `name` 인 context로 전환합니다. 없으면 새로 만든 뒤 그걸 현재로 둡니다. (명령)
  - `Context::Delete(name)`: 이름이 `name` 인 context를 삭제합니다. 현재 context를 지우면 현재는 비거나 남은 것 중 하나가 됩니다. (명령)
- **현재 context에 대한 키–값**
  - `Context::Set(key, value)`: 현재 context에 `key` → `value` 를 넣습니다. (명령)
  - `Context::Get(key)`: 현재 context에서 `key` 에 대한 값을 반환합니다. 없으면 `null` 입니다. (함수)
- **조회**
  - `Context::GetCurrent()`: 현재 context 이름을 반환합니다. (함수)
...
Context::Set("mode", 1)
if (Equal(Context::Get("mode"), 1))
    Talk(JoinString("Current: ", Context::GetCurrent()))
...
### 5.3 호스트 이벤트 오버라이드 (`Event::Override`)

호스트(예: 게임 엔진)가 cm2 와 다른 이벤트 소스(JSON 등)를 함께 사용할 때, cm2 가 이벤트를 처리했음을 알리는 신호입니다. 호출하면 엔진의 `handled` 플래그가 `true` 로 설정되고, 호스트는 그 값을 보고 다른 소스로의 폴백을 건너뜁니다.

**관용**: 매칭된 이벤트 핸들러 블록의 **맨 위**에 두어 "이 블록은 호스트의 다른 소스를 덮어쓴다"는 선언처럼 읽히게 합니다.
...
엔진 측 플래그 자체는 `ScriptEngine.handled` 로 호스트가 매 `run()` 후에 읽고 매 `run()` 시작 시 자동으로 `false` 로 리셋됩니다.
