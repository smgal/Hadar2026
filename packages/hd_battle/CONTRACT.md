...
**확정 2026-09-09** ([W1-06](../../issues/W1-world-core/W1-06-battle-bridge.md)).
v3 은 B6-99 가 얼렸고 W1(RPG 핵심 재작성, 10차 판정)이 다시 열었다.
여기서 벗어나는 변경은 새 판정을 받는다.

> **v3 에서 달라진 것** — 규칙이 아니라 **누가 무엇을 아는가**다.
>
> | 어디 | 무엇 | 왜 |
> |---|---|---|
> | `CombatantSnapshot` | `strikes` · `coatingSlots` · `evasionBonus` · `initiativeBonus` 넷 | 손 구성이 만드는 것을 전투가 알 수 없다 — 전투는 두 손을 보지 않는다 |
> | `Combatant.coatings` | 하나 → **목록**. `coating` 은 첫 칸을 읽는 getter로 남는다 | 쌍수가 칸을 둘 갖는다. 화면이 이미 2 를 보이고 있었다 |
> | 무기 표 | 8줄 추가 — `one_hand_slash` · `sword_shield` · `spear` · `war_hammer` · **`bow` · `crossbow` · `arbalest` · `thrown`** | **원거리가 하나도 없었다.** 활을 든 사냥꾼을 넘길 키가 없었다 |
> | `castableSkills` | 초능력 **41 · 42 · 44 를 뺀다** | 셋 다 전투에서 아무 일도 안 했다 — 헛턴이 메뉴로 도달했다(부록 Z-9) |
>
> **규칙은 하나도 안 바뀌었다.** 피해식·사거리 벌점·대상 가중·밀림은 그대로다.
...
| **v4** | W1-06, 2026-09-09. **손 구성이 만드는 값 넷 · 도포 칸 여럿 · 원거리 무기 · 헛턴 제거** |

바뀔 것으로 이미 아는 것:
- `worldEffects` — **채우는 경로가 전투에 하나도 없다.** 33~40 은 메뉴에 없고
  41·42·44 도 v4 에서 빠졌다(부록 Z-9). 필드는 남기되 쓰이지 않는다 — B3-04 가
  해석하려던 요청이 애초에 발생하지 않는다는 것이 그 이슈의 답이다
